...
之后在 `str` 上调用了一个 `replace` ，[/[\r\t\n]/g](https://regexper.com/#%2F%5B%5Cr%5Ct%5Cn%5D%2Fg) 其实就是全局找 回车符，制表符或者换行符，把它们统一成了空格。
...
又通过 `join('\t')` 的方法把它们拼凑起来了，以制表符为分隔，可以发现现在已经没有 `<%` 符号了
...
其实这里最主要就是找到对应的位置，把模板标识符 `%>` 附近的 `'` 给去掉了，也就是属性上的
